    ...
    <el-dialog v-model="visible" title="修改成绩" @close="$emit('handleClose')">
        <el-form ref="form" label-width="80px">
            ...
            <el-form-item label="Past" v-show="type === 0 || type === 1 || type === 5">
                <el-input v-model="scoreNew.pstScore" />
            </el-form-item>
...
let scoreNew = toRef(scoreData);

//监视更新
watch(scoreData, (newScoreData) => {
    scoreNew.value = toRef(newScoreData);
})

//歌曲搜索
function querySearch(queryString, cb){
    let result;
    console.log(scoreNew.pid);
    result = songs.value.filter(element => {
        return element.sname.toLowerCase().indexOf(queryString.toLowerCase()) !== -1 && ((scoreNew.value.pid === undefined || scoreNew.value.pid === null || scoreNew.value.pid === '')? true: element.pid === scoreNew.value.pid);
    })
    cb(result);
}

function handleSelect(selected){
    scoreNew.value.pid = selected.pid;
}
